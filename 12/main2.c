...
int	main(void)
{
	...
	map = malloc(sizeof(line) * 41);
	y = 0;
	while (y < 41)
	{
		map[y++].text = malloc(180);
	}
	...
	while (list[l].val != 'S' && counter < 1000)
	{
		...
		while (x < z)
		{
			...
			if (temp[x].val == list[l].val || temp[x].val == list[l].val - 1 || temp[x].val == 'S' || (temp[x].val == 'm' && list[l].val == 'k'))
			{
