...
int	main(void)
{
	...
	valves = malloc(sizeof(valve) * 50);
	paths = malloc(sizeof(path) * 50);
	reverse = malloc(sizeof(path) * 50);
	z = 0;
	while (z < 50)
	{
		valves[z].name = malloc(5);
		valves[z].leads = malloc(50);
		z++;
	}
	i = 0;
	l = 0;
	...
	while (time > 0)
	{
		...
		if (travel == 0)
		{
			...
			while (m < npath)
			{
				...
				if (paths[m].score - score > paths[m].flow * paths[m].tt)
				{
